...
public class Enemy_3AI : Agent
{
     ...
     {
        // RollerAgentの落下時
        if (this.transform.localPosition.y < 0 || timelimit < enemy_1.timenow || gamesetflag == true)
        {
            // RollerAgentの位置と速度をリセット
            this._rigidBody.angularVelocity = Vector3.zero;
            this._rigidBody.velocity = Vector3.zero;
            // this.transform.localPosition = new Vector3( 0.0f, 0.5f, 0.0f);
            // this.transform.localPosition = new Vector3(Random.value * Floor_X - Floor_X/2f,
            //                                 20f,
     ...
     public override void CollectObservations(VectorSensor sensor)
     {
       ...
       sensor.AddObservation(Enemy_1.localPosition);
       //
       // // Agent velocity
       ...
       sensor.AddObservation(enemy_1.velocity.x);
       // sensor.AddObservation(velocity.y);
       sensor.AddObservation(enemy_1.velocity.z);
     ...
     public override void OnActionReceived(float[] vectorAction)
     {
       if (gamemanager.game_stop_flg == false){
         ...
         if (gamesetflag == false && distanceToTarget < 0.7f){
           AddReward(30.0f);
           Debug.Log("Enemy3の勝ち！ : " + enemy_1.timenow);
           gamesetflag = true;
           // timenow = 0f;
           //EndEpisode();
         }

         if (gamesetflag == false && (enemy_1.gamesetflag == true || enemy_2.gamesetflag == true)){
           AddReward(5.0f);
           gamesetflag = true;
           //EndEpisode();
         }

         if (timelimit < enemy_1.timenow) {
           AddReward(-5.0f);
           Debug.Log("時間切れ");
           // timenow = 0f;
           EndEpisode();
         ...
         if (gamesetflag == true && enemy_2.gamesetflag == true && enemy_1.gamesetflag == true){
           EndEpisode();
         }
       }
     }
